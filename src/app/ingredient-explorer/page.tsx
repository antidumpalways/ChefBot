...
interface Recipe {
  ...
  cuisine: string
  calories?: number
  isAI?: boolean // Mark AI-generated recipes
}
...
export default function IngredientExplorer() {
  ...
  const generateRecipes = async () => {
    ...
    try {
      // Use Sensay API to generate recipes based on ingredients
      ...
      // Transform Sensay API response to our Recipe format
      const generatedRecipes: Recipe[] = data.recipes.map((recipe: any, index: number) => ({
        id: recipe.id || `sensay_recipe_${Date.now()}_${index}`,
        title: `🤖 ${recipe.title || `AI Recipe ${index + 1}`}`,
        image: getRecipeImage(recipe.title || `Recipe ${index + 1}`, recipe.cuisine || ''),
        prepTime: recipe.prep_time || "30 mins",
        difficulty: recipe.difficulty || "Medium",
        missingIngredients: [],
        availableIngredients: ingredients,
        description: recipe.description || "A delicious AI-generated recipe based on your ingredients!",
      ...
      // Filter static database recipes based on available ingredients
      const staticRecipes = getFilteredStaticRecipes(ingredients)
      const combinedRecipes = [...generatedRecipes, ...staticRecipes]
      ...
      // Fallback to filtered static recipes if API fails
      const fallbackRecipes = getFilteredStaticRecipes(ingredients)
  ...
  const getFilteredStaticRecipes = (availableIngredients: string[]): Recipe[] => {
    const allStaticRecipes = getFallbackRecipes()
    
    console.log('Available ingredients:', availableIngredients)
    console.log('Total static recipes:', allStaticRecipes.length)
    
    // Filter recipes based on available ingredients
    const filteredRecipes = allStaticRecipes.filter(recipe => {
      // Calculate how many ingredients from the recipe are available
      const availableCount = recipe.ingredients.filter(ingredient => 
        availableIngredients.some(available => {
          const availableLower = available.toLowerCase().trim()
          const ingredientLower = ingredient.toLowerCase().trim()
          return availableLower === ingredientLower || 
                 availableLower.includes(ingredientLower) ||
                 ingredientLower.includes(availableLower)
        })
      ).length
      
      console.log(`Recipe: ${recipe.title}, Available: ${availableCount}/${recipe.ingredients.length}`)
      
      // Show recipe if at least 1 ingredient matches (more lenient)
      ...
      return {
        ...
        availableIngredients: availableIngredients.filter(ingredient => 
          recipe.ingredients.some(recipeIngredient => {
            const availableLower = ingredient.toLowerCase().trim()
            const recipeIngredientLower = recipeIngredient.toLowerCase().trim()
            return availableLower === recipeIngredientLower || 
                   availableLower.includes(recipeIngredientLower) ||
                   recipeIngredientLower.includes(availableLower)
          })
        ),
        matchScore: availableCount
      }
  ...
  const getFallbackRecipes = (): Recipe[] => {
    // Fallback static recipes when API is not available
    const fallbackRecipes: Recipe[] = [
        ...
        // Simple and Common Recipes
        {
          id: "scrambled-eggs",
          title: "Scrambled Eggs",
          image: getRecipeImage("scrambled eggs", "American"),
          prepTime: "5 mins",
          difficulty: "Easy",
          missingIngredients: [],
          availableIngredients: [],
          description: "Perfect fluffy scrambled eggs - a breakfast classic! 🥚",
          servings: 2,
          cuisine: "American",
          calories: 140,
          ingredients: [
          ...
          instructions: [
            ...
            "Continue stirring until eggs are just set but still creamy.",
        ...
        },
        {
          ...
          image: getRecipeImage("grilled cheese", "American"),
          prepTime: "10 mins",
          difficulty: "Easy",
          missingIngredients: [],
          availableIngredients: [],
          description: "Crispy, gooey, and absolutely delicious! 🧀🍞",
          servings: 1,
          cuisine: "American",
          calories: 320,
          ingredients: [
          ...
          instructions: [
            ...
            "Cook sandwich for 2-3 minutes per side until golden.",
            "Serve hot and enjoy!"
          ],
        },
        {
          ...
          image: getRecipeImage("pasta aglio olio", "Italian"),
          prepTime: "15 mins",
          difficulty: "Easy",
          missingIngredients: [],
          availableIngredients: [],
          description: "Simple yet elegant - garlic, oil, and pasta perfection! 🍝",
          servings: 2,
          cuisine: "Italian",
          calories: 280,
          ingredients: [
        ...
        },
        {
          ...
          image: getRecipeImage("green salad", "Mediterranean"),
          prepTime: "10 mins",
          difficulty: "Easy",
          missingIngredients: [],
          availableIngredients: [],
          description: "Fresh, crisp, and healthy - the perfect side dish! 🥗",
          servings: 4,
          cuisine: "Mediterranean",
          calories: 80,
          ingredients: [
        ...
        },
        {
          ...
          image: getRecipeImage("vegetable stir fry", "Chinese"),
          prepTime: "15 mins",
          difficulty: "Easy",
          missingIngredients: [],
          availableIngredients: [],
          description: "Quick, healthy, and full of flavor! 🥬",
          servings: 3,
          cuisine: "Chinese",
          calories: 120,
          ingredients: [
                    ...
                    />
                     ...
                     <div className="card-body">
                       <h3 className="card-title text-base-content">
                         {recipe.title}
                         {recipe.isAI && (
                           <span className="badge badge-primary badge-sm ml-2">Sensay Generated</span>
                         )}
                       </h3>
